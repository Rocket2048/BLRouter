...
public class MGJRouter: NSObject {
    ...
    @objc public class func registerWithHandler(_ urlPattern: String, _ toHandler: MGJRouterHandler?) {
    ...
    @objc public class func registerWithObjectHandler(_ urlPattern: String, toObjectHandler: MGJRouterObjectHandler?) {
...
//        print(parameters)
    ...
    /// - Parameter url: 带 Scheme，如 mgj://beauty/3
    /// - Returns: 返回值
    @objc public class func object(_ url: String) -> Any? {
    ...
    public class func generateURL(_ pattern: String, _ parameters: [String]) -> String? {
        ...
        for i in 0..<pattern.count {
            ...
            let endIndex = pattern.index(pattern.startIndex, offsetBy: i+1)
            let character = pattern[index ..< endIndex]
            if character == ":" {
                startIndexOfColon = i
            }
...
extension MGJRouter {
    ...
    func add(_ urlPattern: String) -> NSMutableDictionary? {
        ...
        guard let str = urlPattern.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) else { /////
            return nil
        }
        
        guard let pathComponentsArr = pathComponents(str) else {
            return nil
        }
